...
inModelIntro r1 r2 r3 r4 = foldlSP [S "This", phrase section_, 
  ...
  plural model, S "identified in", (makeRef r3) :+: end]
    where end = S " and" +:+ (mkRefFrmLbl r4)
...
listofTablesToRefsFromLable  [x]    = (mkRefFrmLbl x) +:+ S "shows"
listofTablesToRefsFromLable  [x,y]  = (mkRefFrmLbl x) +:+ S "and" +:+ (mkRefFrmLbl y) +:+ S "show" -- for proper grammar with multiple tables
                                                                                  -- no Oxford comma in case there is only two tables to be referenced
listofTablesToRefsFromLabel  (x:xs) = (mkRefFrmLbl x) `sC` listofTablesToRefsFromLabel (xs)
