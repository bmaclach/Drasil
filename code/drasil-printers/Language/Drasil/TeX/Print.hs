...
spec (Ref _ t@RT.Sect r _ _)    = sref (show t) (pure $ text r)
spec (Ref _ t@(RT.Def _) r _ _) = hyperref (show t) (pure $ text r)
spec (Ref _ RT.Mod r _ _)       = mref  (pure $ text r)
spec (Ref _ (RT.Req _) r _ _)   = rref  (pure $ text r)
spec (Ref _ RT.Assump r _ _)    = aref  (pure $ text r)
spec (Ref _ RT.LCh r _ _)       = lcref (pure $ text r)
spec (Ref _ RT.UnCh r _ _)      = ucref (pure $ text r)
spec (Ref _ RT.Cite r _ _)      = cite  (pure $ text r)
spec (Ref _ RT.Blank r sn _)    = snref r $ spec sn
spec (Ref _ RT.Link r _ sn)     = href  r $ L.getStringSN sn
spec (Ref _ t r _ _)            = ref (show t) (pure $ text r)
spec EmptyS                   = empty
